...
var PaginatableChild = Ember.Mixin.create({
    ...
    addedWatchDog: function () {     
        ...
        if (this.get('model.length') <= thecount) {
            Ember.Logger.info("ADDED LOCAL with %s",this.get('model.length'));
            //this.get('current_results').addObject(this.get('added'));
        } else {
            Ember.Logger.info("ADDED ROUNDTRIP with %s",this.get('model.length'));
    ...
    paginatedContent: function() {
        Ember.Logger.info("PAGINATED STUFF");
    ...
    }.property("model.@each", "sortProperties", "page")
