...
class BaseTests( PpfaWebDriver ):
    ...
    def runTest(self):
        ...
        #Since we recorde every request, let's clear the session
        self.broadcast("Starting Test 'Proof Pilot Arm'")
        ...
        self.broadcast("Entering ARM (34)")
        # find the element that's name attribute is q (the google search box)
        inputElement = self.browser.find_element_by_name("arm")
        
        # type in the search
        inputElement.send_keys("34")
        ...
        for chatpage in chatpages:
            ...
            try:
                ...
                self.failassertion( "CHAT item on", chatpage, "found" )
            except NoSuchElementException:
                self.passassertion( "CHAT item on", chatpage, "not found" )
